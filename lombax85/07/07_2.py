...
class Hand:
    def __init__(self, cards, bid) -> None:
        ...
        # manage j
        if 'J' in self.textCards:
            newCard = self.manageJ()
            # decompose cards
            self.cards = newCard.calcSingleCards()
            # # get the hand type
            self.handType = newCard.calcHandType()

    def manageJ(self):
        # prendo il text della card
        # sostituisco J prima con A, poi con H, poi con Q e così via (tutte le lettere)
        # prendo quella con il type maggiore
        # se ci sono 2 uguali con type maggiore
        options = []
        for k in cardValues.keys():
            if (k != 'J'):
                handText = self.textCards.replace('J', k)
                options.append(Hand(handText, 0))
        
        # get the best option
        options.sort(key=cmp_to_key(cmp), reverse=True)
...
allCards.sort(key=cmp_to_key(cmp))
